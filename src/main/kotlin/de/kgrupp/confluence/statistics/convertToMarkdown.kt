...
private const val MAX_RECENT_BLOG_POSTS_SHOWN = 30
private const val MAX_BLOG_POSTS_PER_AUTHOR_SHOWN = 15

fun List<UserStatistics>.convertToMarkdown(
    minCreatedDate: LocalDate,
    recentBlogPostsMinDate: LocalDate,
): String {
      ...
      this.asSequence()
          ...
          .filter {
            it.createdAt
                ...
                .toLocalDate()
                .isEqualOrAfter(recentBlogPostsMinDate)
          }
          .sortedBy { it.createdAt }
          .sortedBy { -it.likeCount }
          .take(MAX_RECENT_BLOG_POSTS_SHOWN)
          .toList()
  topRecentBlogPosts.forEach { blogPost ->
  ...
  this.forEachIndexed { index, userStatistics ->
    ...
    builder.appendLine()
    userStatistics.popularBlogPosts.take(MAX_BLOG_POSTS_PER_AUTHOR_SHOWN).forEach { blogPost ->
      builder.append("* [${blogPost.title.replaceBreakingCharacters()}](${blogPost.link})")
      if (blogPost.likeCount > 0) {
        builder.append(" | \uD83D\uDC4D ${blogPost.likeCount}")
      }
